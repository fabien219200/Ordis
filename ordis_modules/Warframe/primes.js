...
axios.get("https://api.warframestat.us/warframes/")
    .then(response => {
        var wfTabwPrime = response.data.filter(item => !item.name.includes("Prime"))
        const wfTab = wfTabwPrime.map(item => item.name)
    })

module.exports = {
    name: 'Prime',
    description: 'Gets all relics for requested Prime equipement',
    execute(message) {
        var msg = message.content.replace(/\/+[^ ]* */, "")
        msg = msg.replace(/( +prime)( .*)*/gim, "")
        msg = msg.split(' ')
        msg[msg.length] = "p."
        var wfTab = ["Ash", "Atlas", "Banshee", "Baruuk", "Chroma", "Ember", "Equinox", "Excalibur", "Frost", "Gara", "Garuda", "Gauss", "Grendel", "Harrow", "Hildryn", "Hydroid", "Inaros", "Ivara", "Khora", "Limbo", "Loki", "Mag", "Mesa", "Mirage", "Nekros", "Nezha", "Nidus", "Nova", "Nyx", "Frost", "Oberon", "Octavia", "Odonata", "Protea", "Revenant", "Rhino", "Saryn", "Titania", "Trinity", "Valkyr", "Vauban", "Volt", "Wisp", "Wukong", "Zephyr"]
